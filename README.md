...
automatically when the task fits — you install it once and don't have to
...
- **Claude Code / agent projects:** drop `SKILL.md` into your project's
  skills directory. `commands/blacklight.md` adds a `/blacklight` slash
  command that forces the scan manually and reports the result even when the
  document is clean — a fallback for when you want to check something on
  demand rather than wait for the skill to fire on its own.
